...
def insert_owner(table_name, tuples):
	f = open("insert.sql", "a")
	for i in range(tuples):
		...
		f.write("INSERT INTO " + table_name + " VALUES (" + str(i+1) + ", '" + first + "', '" + last + "', '" + phone + "', '" + email + "', " + str(rating) + ", '" + password + "');\n")
	f.close()

def insert_student(table_name, tuples, temp):
	f = open("insert.sql", "a")
	gender_list, major_list, food_pref_list, gender_pref_list = ['m', 'f', 'o'], ['CSE', 'DS', 'MIS'], ['V', 'NV', 'NA'], ['m', 'f', 'o'] 
	for i in range(tuples):
		first = "".join(random.choices(string.ascii_lowercase, k = random.randint(4, 6)))
		last = "".join(random.choices(string.ascii_lowercase, k = random.randint(5, 8)))
		phone = "".join(random.choices(string.digits, k = 10))
		...
		password = "".join(random.choices(string.ascii_letters, k = random.randint(6, 8)))
		f.write("INSERT INTO " + table_name + " VALUES (" + str(i+1) + ", '" + email + "', '" + first + "', '"
				+ last + "', '" + phone + "', '" + gender + "', '" + major + "', '" + food_pref + "', '" + nationality + "', '" + gender_pref + "', " + str(degree) + ", '" + password + "');\n")
	f.close()

def insert_rating_owner(table_name, tuples):
	f = open("insert.sql", "a")
	stud_id = random.sample(range(1, 1000), tuples)
	owner_id = random.sample(range(1, 1000), tuples)
	for i in range(tuples):
		rate_owner = random.randint(0, 5)
		f.write("INSERT INTO " + table_name + " VALUES (" + str(i+1) + ", " + str(stud_id[i]) +
				", " + str(owner_id[i]) + ", " + str(rate_owner) + ");\n")
	f.close()

def insert_rating_house(table_name, tuples):
	f = open("insert.sql", "a")
	stud_id = random.sample(range(1, 1000), tuples)
	house_id = random.sample(range(1, 1000), tuples)
	for i in range(tuples):
		rate_house = random.randint(0, 5)
		f.write("INSERT INTO " + table_name + " VALUES (" + str(i+1) + ", " + str(stud_id[i]) + ", " + str(house_id[i])
				 + ", " + str(rate_house) + ");\n")
	f.close()

def insert_occupancy(table_name, tuples):
	f = open("insert.sql", "a")
	stud_id = random.sample(range(1, 1000), tuples)
	house_id = random.sample(range(1, 1000), tuples)
	for i in range(tuples):
...
def insert_degree(table_name, tuples):
	f = open("insert.sql", "a")
	for i in range(tuples):
		name = "".join(random.choices(string.ascii_uppercase, k = random.randint(2, 4)))
		f.write("INSERT INTO " + table_name + " VALUES (" + str(i+1) + ", '" + name + "');\n")
	f.close()

def insert_photos(table_name, tuples):
	f = open("insert.sql", "a")
	house_id = random.sample(range(1, 1000), tuples)
	for i in range(tuples):
		name = "".join(random.choices(string.ascii_uppercase, k = random.randint(2, 4)))
		link = "http://www." + "".join(random.choices(string.ascii_lowercase, k = random.randint(8, 12))) + ".com/house-photo"
		f.write("INSERT INTO " + table_name + " VALUES (" + str(i+1) + ", " + str(house_id[i]) + ", '" + link + "');\n")
	f.close()

def insert_sublet(table_name, tuples):
	f = open("insert.sql", "a")
	...
	for i in range(tuples):
		...
		f.write("INSERT INTO " + table_name + " VALUES (" + str(i+1) + ", " + str(sub_from[i]) + ", " + str(sub_to[i])
				+ ", '" + from_date + "', '" + to_data + "');\n")
	f.close()

def insert_house(table_name, tuples):
	f = open("insert.sql", "a")
	...
	for i in range(tuples):
		...
		f.write("INSERT INTO " + table_name + " VALUES (" + str(i+1) + ", " + str(owner_id[i]) + ", "
...
def insert_street(table_name, tuples):
	f = open("insert.sql", "a")
...
insert_owner("House_Owner", 1000)
insert_house("House", 999)
insert_photos("Photo", 999)
# insert_degree("degree", 100)
# insert_student("Student", 1000)
insert_occupancy("Student_Occupancy", 200)
insert_sublet("Sublet", 500)
insert_rating_owner("House_Rating", 500)
insert_rating_house("House_Owner_Rating", 500)
...
insert_student("Student", 1000, temp)